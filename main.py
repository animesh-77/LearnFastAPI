...
# Ideally, we would store this data in a SQL database, but for simplicity we'll
# use a dictionary to store the items in memory
items = {
    0: Item(name="Hammer", price=9.99, count=20, id=0, category=Category.TOOLS),
    1: Item(name="Pliers", price=5.99, count=20, id=1, category=Category.TOOLS),
    2: Item(
        name="Nails", price=1.99, count=100, id=2, category=Category.LIMITED
    ),
}


# FastAPI automatically handles the conversion of Python objects to JSON format
# when sending responses, and converts JSON data to Python objects when receiving requests.
# This simplifies the process of working with JSON data in web applications..
...
Selection = dict[
    str, Union[str, int, float, Category, None]
